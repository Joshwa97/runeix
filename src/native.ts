...
export var native: {
	...
	getBackingScaleFactor: (wnd: BigInt) => number,
...
export function reloadAddon() {
	// When packaged, the addon is in extraResources/native/
	let resourcePath = path.resolve(process.resourcesPath || "", "native/addon.node");
	let releasePath = path.resolve(__dirname, "../build/Release/addon.node");
	let debugPath = path.resolve(__dirname, "../build/Debug/addon.node");
	let addonpath = fs.existsSync(resourcePath) ? resourcePath
		: fs.existsSync(releasePath) ? releasePath : debugPath;

	//Copy the addon file so we can rebuild while running
	if (process.env.NODE_ENV === "development") {
		let dir = path.dirname(addonpath);
		let tmpfile = path.resolve(dir, "addon" + Math.floor(Math.random() * 1000) + ".node");
		fs.copyFileSync(addonpath, tmpfile);
		addonpath = tmpfile;
	}
